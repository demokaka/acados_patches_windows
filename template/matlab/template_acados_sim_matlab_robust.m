...
%
% IMPORTANT:
% The acados hash/JSON reuse check verifies the formulation, but it does not
% guarantee that every generated artifact still exists. This matters after
% an interrupted/failed generation: the JSON may exist and match while the
% generated wrapper .m or shared library is missing.
%
% Therefore this template checks BOTH:
%
%   (A) formulation reuse via acados JSON/hash
%   (B) required generated artifacts on disk
...
% -------------------------------------------------------------------------
% 9.1 Generic SIM MEX interface: sim_solve / sim_get
% -------------------------------------------------------------------------
%
% Current MATLAB acados auto-detection checks for sim_create.<mexext>,
% although sim_compile_interface() produces sim_solve and sim_get.
% Detect the actual files ourselves so they are not recompiled every run.

sim_interface_dir = fullfile(pwd, 'build');

if ~isfolder(sim_interface_dir)
    mkdir(sim_interface_dir);
end

sim_solve_mex = fullfile( ...
    sim_interface_dir, ...
    ['sim_solve.' mexext]);

sim_get_mex = fullfile( ...
    sim_interface_dir, ...
    ['sim_get.' mexext]);

opts.output_dir = sim_interface_dir;

opts.compile_interface = ...
    ~(isfile(sim_solve_mex) && isfile(sim_get_mex));


% -------------------------------------------------------------------------
% 9.2 Check generated problem-specific artifacts
% -------------------------------------------------------------------------

sim_wrapper_m = fullfile( ...
    codegen_dir, ...
    [sim.name '_mex_sim_solver.m']);

if ispc
    sim_shared_lib = fullfile( ...
        codegen_dir, ...
        ['acados_sim_solver_' sim.name '.dll']);
elseif ismac
    sim_shared_lib = fullfile( ...
        codegen_dir, ...
        ['libacados_sim_solver_' sim.name '.dylib']);
else
    sim_shared_lib = fullfile( ...
        codegen_dir, ...
        ['libacados_sim_solver_' sim.name '.so']);
end


% If the generated wrapper is missing, rendering did not complete.
% Force a complete regenerate + rebuild even if JSON/hash says "reuse".
if ~isfile(sim_wrapper_m)

    disp('Generated SIM wrapper is missing -> forcing regeneration.');

    opts.generate = true;
    opts.build = true;

% If generated code exists but the problem-specific shared library is
% missing, only the native build is required.
elseif ~isfile(sim_shared_lib)

    disp('SIM shared library is missing -> forcing rebuild.');
